...
export class VDouyinService {
    ...
    log(...args:any[]){
     ...
     async parseWatermark(url:string,openid:string,originUrl:string){
        ...
        this.log("id:",id);
        if(!id){
            throw new BadRequestException('视频ID不存在');
        }
        ...
        if (!matches || !matches[1]) {
            throw new BadRequestException('解析数据失败');
        }
         let videoInfo: any;
        try {
            ...
            throw new BadRequestException(`JSON 解析失败: ${error instanceof Error ? error.message : String(error)}`);
        }
        console.log(videoInfo);

        if (!videoInfo?.loaderData) {
            console.log('数据查找失败' + response );
            throw new BadRequestException('数据查找失败');
        }

        const itemList = videoInfo.loaderData['video_(id)/page']?.videoInfoRes?.item_list?.[0];
        if (!itemList) {
            console.log("视频信息不存在")
            throw new BadRequestException('视频信息不存在');
        }
        ...
        if (!videoResUrl && imgurl.length === 0) {
            ...
            throw new BadRequestException('当前分享链接已失效！');
        }
